...
<li><a href="../../../index.html">홈</a></li>
                ...
                <li title="현재 위치"><strong>상품 자유게시판</strong></li>
            </ol>
</div>
<div class="titleArea">
...
<tbody class="xans-element- xans-board xans-board-list-1002 xans-board-list xans-board-1002 center">
           ...
           <% if(boardList.isEmpty()) { %>
			...
			<tr style="background-color:#FFFFFF; color:#555555;" class="xans-record-">
            <td>
            <input type="hidden" name="num" value="<%=board.getB_no() %>"></input>
           	<%=number %></td>
		    ...
		    <% } %>
           	...
           	<td class="subject left txtBreak">
        		<% if(board.getRestep()!=0) { %>
	   				<span style="margin-left: <%=board.getRelevel()*20%>px;"><img src="http://img.echosting.cafe24.com/design/skin/admin/ko_KR/ico_re.gif" alt="답변" class="ec-common-rwd-image" /></span> 
	   			<% } %>        
	    		<% if(board.getStatus()==0) { %>
			    	<span class="txtEm">	   		   	
					<a href="<%=request.getContextPath()%>/board/freeboard/board_detail.jsp?b_no=<%=board.getB_no() %>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>" style="color:#555555;"><%=board.getB_title() %></a></span>
		   		<% } else if(board.getStatus()==9) { %>
		    		<span class="txtEm">삭제된 게시글 입니다.</span> 
		   		<% } %>			
		    </td>
            <td><input type="hidden" name="m_userid" value="<%=board.getM_userid()%>">
   	 		<% MemberDTO member=MemberDAO.getDAO().memberSelect(board.getM_userid(),"");%>
    		<%=member.getM_name()%></input></td>
    
            <td class=""><span class="txtNum"><%=board.getB_date()%></span></td>
  			<td class=""><span class="txtNum"><%=board.getB_count()%></span>
...
<button type="button" class="btnSubmitFix sizeS" onclick="location.href='<%=request.getContextPath() %>/board/freeboard/freeboard_write.jsp'">글쓰기</button>
        </span>
</div>
</div>
...
<a href="<%=request.getContextPath()%>/board/freeboard/freeboard.jsp?pageNum=1&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_first.gif" alt="첫 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/freeboard/freeboard.jsp?pageNum=<%=startblockPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
...
<%for(int i=startblockPage;i<=endblockPage;i++) { %>
	...
	<a href="<%=request.getContextPath()%>/board/freeboard/board.jsp?pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>" class="none"><%=i%></a>
	</li>
	<% } else { %>
...
<a href="<%=request.getContextPath()%>/board/freeboard/freeboard.jsp?pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">
