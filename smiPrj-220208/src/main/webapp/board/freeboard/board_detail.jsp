...
*/
	...
	*/
	...
	BoardDAO.getDAO().updateBcount(b_no);
%> 
...
<form id="BoardDelForm" name="BoardDel" action="<%=request.getContextPath() %>/board/freeboard/board_modify.jsp" method="post" enctype="multipart/form-data" >
...
<tr>
<th scope="row">제목</th>
                    <td><input type="hidden" id="b_no" value="<%=board.getB_no()%>"/><%=board.getB_title() %></td>
                </tr>
<tr>
<th scope="row">작성자</th>
					<td>
...
<td colspan="2">
                        <ul class="etcArea">
                            <li id="count">
				            <strong>작성일</strong> <span class="txtNum"><%=board.getB_date() %></span>
				        </li>
				        <li id="count">
				            <strong>조회수</strong> <span class="txtNum"><%=board.getB_count()+1 %></span>
				        </li>
                        </ul>
                            <br></br>
    <br></br>
    <br></br>
<div class="detail"><div class="fr-view fr-view-article"><p><%=board.getB_content()%></p>    <br></br>
    <br></br>
    <br></br></div></div>
                    </td>
                </tr>
...
<th scope="row">첨부파일</th>
                    <td><img src="<%=request.getContextPath()%>/web/images/board-img/<%=board.getB_img()%>" width="100" height="100"></td>
                </tr>
...
<div class="ec-base-button ">
            <span class="gLeft">
                <a href="<%=request.getContextPath() %>/board/freeboard/freeboard.jsp" class="btnNormalFix sizeS">목록</a>
            </span>
            <span class="gRight">
...
$("#replyBtn").click(function() {
	$("#BoardDelForm").attr("action", "<%=request.getContextPath()%>/board/freeboard/board_write.jsp");
	$("#BoardDelForm").submit();
});
$("#modBtn").click(function() {
	var value=$("#b_no").val();
	$("#BoardDelForm").attr("action", "<%=request.getContextPath()%>/board/freeboard/board_modify.jsp?b_no="+value);
	$("#BoardDelForm").submit();
});
$("#removeBtn").click(function() {
	var value=$("#b_no").val();
	$("#BoardDelForm").attr("action", "<%=request.getContextPath()%>/board/freeboard/board_remove_action.jsp?b_no="+value);
	$("#BoardDelForm").submit();
});
</script>
</body></html>
