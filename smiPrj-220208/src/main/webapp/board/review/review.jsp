...
<tbody class="xans-element- xans-board xans-board-list-1002 xans-board-list xans-board-1002 center">
           ...
           <% if(reviewList.isEmpty()) { %>
			...
			<tr style="background-color:#FFFFFF; color:#555555;" class="xans-record-">
            <td>
            <input type="hidden" name="num" value="<%=review.getR_no() %>"></input>
           	<%=number %></td>
		    ...
		    <% } %>
           	...
           	<td class="subject left txtBreak">
        		<% if(review.getRestep()!=0) { %>
	   				<span style="margin-left: <%=review.getRelevel()*20%>px;"><img src="http://img.echosting.cafe24.com/design/skin/admin/ko_KR/ico_re.gif" alt="답변" class="ec-common-rwd-image" /></span> 
	   			<% } %>        
	    		<% if(review.getStatus()==0) { %>
			    	<span class="txtEm">	   		   	
					<a href="<%=request.getContextPath()%>/board/review/review_detail.jsp?r_no=<%=review.getR_no() %>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>" style="color:#555555;"><%=review.getR_title() %></a></span>
		   		<% } else if(review.getStatus()==9) { %>
		    		<span class="txtEm">삭제된 게시글 입니다.</span> 
		   		<% } %>			
		    </td>
            <td><input type="hidden"  name="m_userid" value="<%=review.getM_userid()%>">
   	 		<% MemberDTO member=MemberDAO.getDAO().memberSelect(review.getM_userid(),"");%>
    		<%=member.getM_name()%></input></td>
    
            <td class=""><span class="txtNum"><%=review.getR_date()%></span></td>
  			<td class=""><span class="txtNum"><%=review.getR_count()%></span>
...
<div class="xans-element- xans-board xans-board-buttonlist-4 xans-board-buttonlist xans-board-4  ec-base-button typeBG "><span class="gRight">
            <button type="button" class="btnSubmitFix sizeS" onclick="location.href='<%=request.getContextPath() %>/board/review/review_write.jsp'" >글쓰기</button>
        </span>
</div>
</div>
<div class="xans-element- xans-board xans-board-paging-1002 xans-board-paging xans-board-1002 ec-base-paginate">
<% if(startblockPage>pageSize) { %>
<a href="<%=request.getContextPath()%>/board/review/review.jsp?pageNum=1&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_first.gif" alt="첫 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/review/review.jsp?pageNum=<%=startRow-pageSize%>&search=<%=search%>&keyword=<%=keyword%>">
...
<%for(int i=startblockPage;i<=endblockPage;i++) { %>
	...
	<a href="<%=request.getContextPath()%>/board/review/review.jsp?pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>" class="none"><%=i%></a>
	</li>
	<% } else { %>
...
<% } %>
<% if(endblockPage!=totalPage) { %>
<a href="<%=request.getContextPath()%>/board/review/review.jsp?pageNum=<%=startblockPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_next.gif" alt="다음 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/review/review.jsp?pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">
