...
<%if(ref.equals("0")) { %>
            ...
            <p>상품 사용후기입니다.</p>
            <% } else { %>
            <h2><font color="333333">사용후기 답글</font></h2>
            <p>사용후기 답글입니다.</p>
            <% } %>            
        </div>
</div>
<form id="reviewModifyForm" name="reviewModifyForm" action="<%=request.getContextPath() %>/board/review/review_modify_action.jsp?r_no=<%=review.getR_no() %>" method="post" enctype="multipart/form-data">
...
<th scope="row">제목</th>
                    <td> 
                    <input id="r_title" name="r_title" fw-filter="isFill" fw-label="제목" fw-msg="" class="inputTypeText" placeholder="" maxLength="125" value="<%=review.getR_title() %>" type="text"  />  </td>
                </tr>
<tr class="display">
<th scope="row">작성자</th>
...
<td colspan="2" class="clear">
            ...
            <!-- HTML-->
            <textarea style="width: 100%;" name="content" id="content" class="ec-fr-never-be-duplicated"><%=review.getR_content() %></textarea>
                <input type="hidden" id="content" fw-filter="isSimplexEditorFill" fw-label="내용" value="EC_FROALA_INSTANCE" />
...
<th scope="row">첨부파일1</th>
                    <td><img src="<%=request.getContextPath()%>/web/images/review-img/<%=review.getR_img()%>" width="100" height="100">
...
<div class="ec-base-button ">
            ...
            <span class="gRight">
              ...
              <!-- <a href="javascript:history.back();" class="btnBasicFix sizeS">취소</a> -->
            	<a href="<%=request.getContextPath()%>/board/review/reivew.jsp" class="btnBasicFix sizeS">취소</a>
            </span>
        </div>
</div>
</form></div>
...
$("#modBtn").click(function() {
	...
	$("#reviewModifyForm").submit(function() {
		if($("#r_title").val()=="") {
