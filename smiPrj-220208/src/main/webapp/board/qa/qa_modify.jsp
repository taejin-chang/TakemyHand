...
<%if(ref.equals("0")) { %>
            ...
            <p>상품 Q&A입니다.</p>
            <% } else { %>
            <h2><font color="333333">Q&A 답글</font></h2>
            <p>Q&A 답글입니다.</p>
            <% } %>
        </div>
</div>
<form id="boardWriteForm" name="boardWriteForm" action="<%=request.getContextPath()%>/board/qa/qa_mod_action.jsp?q_no=<%=qa.getQ_no()%>" method="post" target="_self">
...
<input id="ce48e3734d95d9bca" name="ce48e3734d95d9bca" value="ff6ecb518cec001486dd93463ad5e0c7" type="hidden"  /><div class="xans-element- xans-board xans-board-write-4 xans-board-write xans-board-4">
...
<th scope="row">제목</th>
                    <td> 
<input id="q_title" name="q_title" fw-filter="isFill" fw-label="제목" fw-msg="" class="inputTypeText"  maxLength="125" type="text" value="<%=qa.getQ_title()%>" /></div>
</td>
                </tr>
<tr class="display">
<th scope="row">작성자</th>
...
<td colspan="2" class="clear">
            ...
            <!-- HTML -->
            <textarea style="width: 100%;" name="content" id="content" class="ec-fr-never-be-duplicated"><%=qa.getQ_content()%></textarea>
                <input type="hidden" id="q_content" fw-filter="isSimplexEditorFill" fw-label="내용" value="EC_FROALA_INSTANCE"/>
...
<th scope="row">비밀글설정</th>
                    <td><input id="q_status" name="q_status" fw-filter="isFill" fw-label="비밀글설정" fw-msg="" value="0" type="radio" checked="checked"  /><label for="secure0" >공개글</label>
<input id="q_status" name="q_status" fw-filter="isFill" fw-label="비밀글설정" fw-msg="" value="1" type="radio"  /><label for="secure1" >비밀글</label></td>
                </tr>
<tr class="captcha displaynone">
<th scope="row">보안문자</th>
                    <td>
                                                 <p class="gBlank5"> <span class="ec-base-help txtInfo">영문, 숫자 조합을 공백없이 입력하세요(대소문자구분)</span></p>
                    </td>
                </tr>
...
<div class="ec-base-button ">
            ...
            <span class="gRight">
                ...
                <!--  <a href="<%=request.getContextPath()%>/board/qa/qa_add_action.jsp?m_userid=<%=loginMember.getM_userid() %>" type="submit" class="btnSubmitFix sizeS">등록</a>-->
                <a href="<%=request.getContextPath()%>/board/qa/qa.jsp" class="btnBasicFix sizeS">취소</a>
            </span>
        </div>
</div>
<input name="_subject" id="tmp_subject" value="asd" type="hidden"  /><textarea name="_message" id="tmp_content" style="display:none" >&lt;p&gt;asd&lt;/p&gt;</textarea>
</form></div>
