...
<%
	...
	int totalQa=QuestionDAO.getDAO().selectCountQna(search, keyword);
...
(function(i, s, o, g, r, a, m) {
    ...
    a.src = g;
    a.onload = function() {
        i[r].init('https://js-error-tracer-api.cafe24.com', {"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..qmreS_pVsMMR1R0WVthjEm1vo5i5Vt8LGIeOc6GzB4Q","collectWindowErrors":true,"preventDuplicateReport":true,"storageKeyPrefix":"EC_JET.BOARD_PRODUCT_LIST"});
    };
...
<script type="text/javascript" src="http://ecudemo200642.cafe24.com/app/Eclog/js/cid.generate.js?vs=90ea239e17bbbf0c278f17c385b31034"></script>
...
<link rel="stylesheet" type="text/css" href="http://ecudemo200642.cafe24.com/ind-script/optimizer.php?filename=nZBBCkIxEEP3xa3nCHoEj-ANpnXUD-2kTKegt9edbgTpNuTlQXBnUxyOju68uTS4Dk4vijIGrk4LFLZG272DPf7pa0mDdcZGS5mPRXBGrEqrPNXX0JBcdRFlT3UzTVnMfvqld5wofsH5e-7z8Qs&type=css&k=a052fbc291df183c9caddf6aefc2c6be24ab00a6&t=1635315871"  /><link rel="stylesheet" type="text/css" href="http://ecudemo200642.cafe24.com/ind-script/optimizer.php?filename=tZZNbgMhDIX3mW57DqKo6hm66KJSTwDGA6iAET9Rcvt6mqoXqL1DGD5bT88GE6mgQdjWwD54sTwWupzPry8X05bLCbY4SzbD4-ZxpFCNswMNjGEK-ZXROLLdm5zG_LDwZQM-cfDZiJEH2g5RGAp28s2rdK37qjAT1f9hs73Tmsc6wU8OoFLEoY8NYSjC6QicVlICT6I8U1OiR8xa6NbZKKCld7MhVfa0lurWKZHdmlPc2n9wuukJkrXE5kOghOYAdrGZ95umLO7IEal9xtRaqkE6AVC9MoPnagNxNjdNoH6X5j5erTe0Xl7wTIEmNfGSJ2txsPddDV3VzPfOH5AD_g0&type=css&k=8d5c608252cc90a0a086ec88e9c0d57e17a69eba&t=1636193578&user=T"  />
...
<tr style="background-color:#FFFFFF; color:#555555;" class="xans-record-">
	<td>
	<input type="hidden" name="num" value="<%=qa.getQ_no()%>"></input>
	<%=number %></td>
	
	<%if(product!=null) { %>
	<td class="thumb left">
	<a href="<%=request.getContextPath()%>/product/product_datail.jsp?p_no=<%=p_no%>">
	<img src="<%=request.getContextPath()%>/web/images/<%=product.getP_kinds()%>/<%=product.getP_imgsrc()%>"  border="0" alt="" />
	<span><%=product.getP_name() %></span></a></td>
    ...
    <td class="subject left txtBreak">
	    <% if(qa.getReStep()!=0) { %>
	   		<span style="margin-left: <%=qa.getReLevel()*20%>px;"><img src="http://img.echosting.cafe24.com/design/skin/admin/ko_KR/ico_re.gif" alt="답변" class="ec-common-rwd-image" /></span> 
	    <% } %>
	    <% if(qa.getQ_status().equals("0")) { %>
		    <span class="txtEm">
		    <a href="<%=request.getContextPath()%>/board/qa/qa_detail.jsp?q_no=<%=qa.getQ_no()%>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>" style="color:#555555;"><%=qa.getQ_title()%></a></span>
	    <% } else if(qa.getQ_status().equals("1")) { %>
	    	<span class="txtEm"><%=qa.getQ_title()%>
	    	<a href="<%=request.getContextPath()%>/board/qa/qa_detail.jsp?q_no=<%=qa.getQ_no()%>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>" style="color:#555555;"></a></span>
	    <% } else if(qa.getQ_status().equals("9")) { %>
	    	<span class="txtEm">삭제된 게시글 입니다.</span> 
	    <% } %>
    </td>
    <td><input type="hidden" name="m_userid" value="<%=qa.getM_userid()%>">
    <% MemberDTO member=MemberDAO.getDAO().memberSelect(qa.getM_userid(),"");%>
    <%=member.getM_name()%></input></td>
    
    <td class=""><span class="txtNum"><%=qa.getQ_date()%></span></td>
    <td class=""><span class="txtNum"><%=qa.getQ_count()%></span>
    <% number --; %>
    </td>
	</tr>
	<% } %>
...
<button type="button" class="btnSubmitFix sizeS" onclick="location.href='<%=request.getContextPath()%>/board/qa/qa_add.jsp'">글쓰기</button>
        </span>
</div>
</div>
...
<a href="<%=request.getContextPath()%>/board/qa/qa.jsp?pageNum=1&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_first.gif" alt="첫 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/qa/qa.jsp?pageNum=<%=startblockPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
...
<%for(int i=startblockPage;i<=endblockPage;i++) { %>
	...
	<a href="<%=request.getContextPath()%>/board/qa/qa.jsp?pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>" class="none"><%=i%></a>
	</li>
	<% } else { %>
	<li class="xans-record-"><a class="this"><%=i%></a></li>
	</ol>
	<% } %>
<% } %>
...
<% if(endblockPage!=totalPage) { %>
<a href="<%=request.getContextPath()%>/board/qa/qa.jsp?pageNum=<%=startblockPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_next.gif" alt="다음 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/qa/qa.jsp?pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_last.gif" alt="마지막 페이지"/></a>
<% } else { %>
<a href="">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_next.gif" alt="다음 페이지"/></a>
<a href="">
...
<input id="keyword" name="keyword" fw-filter="" fw-label="" fw-msg="" class="inputTypeText" placeholder="" value="" type="text"  /> 
<button class="btnEmFix" type="submit">찾기</button></p>
