...
(function(i, s, o, g, r, a, m) {
    ...
    a.src = g;
    a.onload = function() {
        i[r].init('https://js-error-tracer-api.cafe24.com', {"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..lMFxIptsK6tclbcFBO_x-AFzJEOOfgUpzkhnjNWeD1o","collectWindowErrors":true,"preventDuplicateReport":true,"storageKeyPrefix":"EC_JET.BOARD_FREE_LIST"});
    };
...
<% } else { %>
	<% for(NoticeDTO notice:noticeList) { %>
		<tr style="background-color:#FFFFFF; color:#555555;" class="xans-record-">
			<td>
				<input type="hidden" name="num" value="<%=notice.getN_no()%>"></input>
				<%=number %></td>
		    <td class="subject left txtBreak">
			    <a href="<%=request.getContextPath()%>/board/notice/notice_detail.jsp?n_no=<%=notice.getN_no()%>&pageNum=<%=pageNum %>&search=<%=search%>&keyword=<%=keyword%>" style="color:#555555;">
			    <%=notice.getN_title()%></a> <span class="txtEm"></span>
		    </td>
		    <td><input type="hidden"  name="m_userid" value="<%=notice.getM_userid()%>"></input>
		    	admin</td>
		    <td class=""><span class="txtNum"><%=notice.getN_regdate()%></span></td>
		    <td class=""><span class="txtNum"><%=notice.getN_count()%></span>
		    <% number --; %>
		    </td>
		</tr>
	<% } %>
...
<a href="<%=request.getContextPath()%>/board/notice/notice.jsp?pageNum=1&search=<%=search%>&keyword=<%=keyword%>">
<img src="//img.echosting.cafe24.com/skin/base/common/btn_page_first.gif" alt="첫 페이지"/></a>
<a href="<%=request.getContextPath()%>/board/notice/notice.jsp?pageNum=<%=startblockPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>">
...
<%for(int i=startblockPage;i<=endblockPage;i++) { %>
	...
	<a href="<%=request.getContextPath()%>/board/notice/notice.jsp?pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>" class="none"><%=i%></a>
	</li>
	<% } else { %>
...
<a href="<%=request.getContextPath()%>/board/notice/notice.jsp?pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>">
...
<form id="boardSearchForm" name="" action="<%=request.getContextPath()%>/board/notice/notice.jsp" method="post">
...
<legend>게시물 검색</legend>
        ...
        <p><select id="search" name="search" >
